...
plt.title("CTE vs Time (Filtered Runs)")
plt.grid(True)
plt.legend()
...
# # -----------------------------
# # Compute CTE (distance to polyline)
# # -----------------------------
# def compute_cte(robot_pos, path):
#     robot_pos = np.array(robot_pos)
#     min_dist = float("inf")
...
#         # robot pose
#         # -------------------------
#         if topic == "/odom": # pf/pose/odom when running on the car
#             x = msg.pose.pose.position.x
#             y = msg.pose.pose.position.y
#             poses.append((t, np.array([x, y])))
...
#     return poses, np.array(trajectory)
...
#     for t, pos in poses:
#         cte = compute_cte(pos, traj)
#         times.append((t - t0) * 1e-9)  # ns → seconds
#         cte_values.append(cte)
...
# plt.xlabel("Time (s)")
# plt.ylabel("Cross-Track Error (m)")
# plt.title("CTE vs Time (PoseArray Trajectory)")
# plt.grid(True)
# plt.legend()
